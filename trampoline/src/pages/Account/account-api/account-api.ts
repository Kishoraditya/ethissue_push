...
class TwoOwnerAccountAPI extends AccountApiType {
  ...
  async signUserOpWithContext(
    ...
    // Implement your logic here to sign the user operation with context
    // You can use the provided userOp and context parameters to sign the user operation.
    // Return the signed user operation.
    const signature = await this.signOwnerOne(await this.getUserOpHash(userOp));
    return {
      ...userOp,
      signature: ethers.utils.defaultAbiCoder.encode(
        ['bytes', 'bytes'],
        [signature, context.signedMessage]
      ),
    };
  }

  // Inside TwoOwnerAccountAPI class
...
async createUnsignedUserOpWithContext(
  ...
  // Implement your logic here to create an unsigned user operation with context
  // You can use the provided info and context parameters to create the user operation.
  // Return the created user operation.
  const userOp = await super.createUnsignedUserOp(info);
  userOp.preVerificationGas = BigNumber.from(userOp.preVerificationGas).mul(2.5);
  return {
    ...userOp,
    signature: ethers.utils.defaultAbiCoder.encode(
      ['bytes', 'bytes'],
      ['', context.signedMessage] // You need to provide the correct bytes here
    ),
  };
}
async signUserOpHash(userOpHash: string): Promise<string> {
  // Calculate the hash using the provided userOpHash (already a string)
  const hash = keccak256(userOpHash);
